...
import {formatDuration, limitTextLength} from "@/presentation/pages/utils.ts";
            ...
            <div className="p-6">
                ...
                {repairHistory.length > 0 ? (
                            ...
                            >
                                <div className="flex items-start justify-between mb-2">
                                    <div className="flex-1">
                                        <p className="text-sm text-slate-900">{limitTextLength(repair.description, 35)}</p>
                                    ...
                                    {repair.stateHistory[repair.stateHistory.length - 1].status === 'finished' && (
                                        <CheckCircle2 className="w-4 h-4 text-green-500 shrink-0" />
                                    )}
                                </div>
                                <div className="flex items-center gap-2 text-xs text-slate-600">
                                    <User className="w-3 h-3 text-slate-400" />
                                    <span>{repair.stateHistory[repair.stateHistory.length - 1].responsibleUser?.username}</span>
                                </div>
                                {repair.stateHistory[repair.stateHistory.length - 1].status === 'finished' && (() => {{console.log(repair)}; return true})() && (
                    ...
                    <div className="text-center py-6">
                        ...
                        <p className="text-sm text-slate-500">Немає попередніх ремонтів</p>
                    </div>
                )}
            </div>
        </Card>
